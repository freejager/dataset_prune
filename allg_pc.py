...
def main():
    ...
    trainloader_new=train_double(args, net1, net2, net3, net4, net5, trainloader1,trainloader2, trainloader3, trainloader4, trainloader5, train_data)
...
def get_loss(loader, args, model_else): 
    lbd = args.lbd
    all_loss = []
...
def train_double(args, model1, model2, model3, model4, model5, trainloader1, trainloader2, trainloader3, trainloader4, trainloader5, train_data):
    lbd = args.lbd
    all_sample_losses_1 = [] 
    all_sample_losses_2 = [] 
    all_sample_losses_3 = [] 
    ...
    all_sample_losses_1 = get_loss(trainloader1, args, model1)
    all_sample_losses_2 = get_loss(trainloader2, args, model2)
    all_sample_losses_3 = get_loss(trainloader3, args, model3)
    all_sample_losses_4 = get_loss(trainloader4, args, model4)
    all_sample_losses_5 = get_loss(trainloader5, args, model5)
    ...
    keep_rate = args.rate
    per_class = [0 for _ in range(args.num_classes)] 
    per_class_num = int((args.num_samples / args.num_classes) * args.rate)
    print("per_class_num:",per_class_num)
    
    sample_num = 0
    
    sorted_indices = []
    last = [per_class_num for _ in range(10)]
    for _, index in all_sample_losses:
        sample_num += 1
        tar = train_data.target[index]
        #print(f"index:{index},tar:{tar}")
        if per_class[tar] < per_class_num:
            per_class[tar] += 1
