...
parser.add_argument('--random_rate', type=float, default=0.5, help='random_rate')
...
def train_double(args,  all_sample_losses_1, all_sample_losses_2, all_sample_losses_3, all_sample_losses_4, all_sample_losses_5, train_data):
    ...
    total_count = len(all_sample_losses)
    random_sample = random.sample(all_sample_losses, int(total_count * args.rate * args.random_rate))
    print("random_sample:",random_sample)
    print("len(random_sample):",len(random_sample))
    sorted_indices = [index for _, index in random_sample]
    print("len(sorted_indices:):",len(sorted_indices))

    all_sample_losses_later = [(loss, index) for (loss, index) in all_sample_losses if (loss, index) not in random_sample]
    print("len(all_sample_losses):",len(all_sample_losses_later))
    all_sample_losses_later.sort(key=lambda x: x[0])  
    keep_rate = args.rate
    
    num_to_keep = int(len(all_sample_losses) * keep_rate * (1 - args.random_rate))
    print("num_to_keep:",num_to_keep)
    sorted_indices += [index for _, index in all_sample_losses_later[:num_to_keep]]
    print("len(sorted_indices_later)::",len(sorted_indices))
