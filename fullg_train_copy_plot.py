...
def main():
    ...
    trainloader_new, keep_indices=train_double(args, net, net1, net2, net3, net4, net5, trainloader1,trainloader2, trainloader3, trainloader4, trainloader5, train_data)
    ...
    plt.scatter(tsne_results[mask, 0], tsne_results[mask, 1], c='blue', label='Pruned', alpha=0.5, s=0.5)
    plt.scatter(tsne_results[~mask, 0], tsne_results[~mask, 1], c='red', label='Retained', alpha=0.5, s=0.5)
    plt.legend()
    plt.title("t-SNE Visualization of CIFAR-10 Categories")
    plt.savefig(f'cifar10_fullg_{args.lbd}_{args.rate}.png')
    plt.show()
    print("fig save over")
...
def train_double(args, model, model1, model2, model3, model4, model5, trainloader1, trainloader2, trainloader3, trainloader4, trainloader5, train_data):
    lbd = args.lbd
    all_sample_losses_1 = [] 
    all_sample_losses_2 = [] 
    all_sample_losses_3 = [] 
    ...
    all_sample_losses_1 = get_loss(trainloader1, args, model, model1)
    all_sample_losses_2 = get_loss(trainloader2, args, model, model2)
    all_sample_losses_3 = get_loss(trainloader3, args, model, model3)
    all_sample_losses_4 = get_loss(trainloader4, args, model, model4)
    all_sample_losses_5 = get_loss(trainloader5, args, model, model5)
    ...
    keep_rate = args.rate
    sorted_indices = [index for _, index in all_sample_losses[:int(len(all_sample_losses) * keep_rate)]]
    ...
    return trainloader_new, sorted_indices
